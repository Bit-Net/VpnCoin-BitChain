...
public:
    ...
    std::string ToString() const
    {
        ...
        if (prevout.IsNull())
            str += strprintf(", coinbase [%s] \n [%s]", HexStr(scriptSig).c_str(), scriptSig.ToString().c_str());
        else
...
public:
    ...
    */
    ...
    std::string ToString() const
    {
        ...
        str += strprintf("(hash=%s, nTime=%d, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%d, vpndata=%s)\n",
            GetHash().ToString().substr(0,10).c_str(),
            nTime,
            nVersion,
            vin.size(),
            vout.size(),
            nLockTime, vpndata.c_str());
