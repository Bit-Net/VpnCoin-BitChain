...
//printf("BitchainPage::gotoPage sBitChainIdentAddress = [%s] [%s]\n", sBitChainIdentAddress.c_str(), sEn.c_str());
    ...
    string sUrl = strprintf( "http://%s/?pid=%d&tm=""%" PRId64 "&addr=%s&zm=%.2f&lang=%s&ttx=%s&sign=%s", sQkl_domain.c_str(), pageId, tm, sBitChainIdentAddress.c_str(), DOB_DPI_RAT, sEn.c_str(), sTopic.c_str(), sSig.c_str());
