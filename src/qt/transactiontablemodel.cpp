...
{
    columns << QString() << tr("Date") << tr("Type") << tr("Address") << tr("Transaction comment") << tr("Amount");
...
QString TransactionTableModel::formatTxComment(const TransactionRecord *wtx, bool tooltip) const
{
    ...
    std::string sBcp = getBitChainProtoDescribeStr(wtx->txcomment);  //wtx->time,  wtx->hash
	if( sBcp.length() > 0 ){ strTxComment = QString::fromStdString(sBcp); }
