...
MultisigDialog::MultisigDialog(QWidget *parent) : QWidget(parent), ui(new Ui::MultisigDialog), model(0)
{
    ...
    ui->saveRedeemScriptButton->setVisible(false);
    ui->saveMultisigAddressButton->setVisible(false);
}
...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    for(int i = 0; i < ui->pubkeyEntries->count(); i++)
    {
        ...
        if (!vchPubKey.IsValid() || !pubkeys[i].SetPubKey(vchPubKey))   //if(!vchPubKey.IsValid())
            return;
        //pubkeys[i] = vchPubKey;
    }

    QString q = tr("Number of public key");   // Please reduce the number
    if(pubkeys.size() > 16)
    {
        QMessageBox::warning(this, tr("Error"), q + " > 16 ", QMessageBox::Ok);   //QMessageBox::warning(this, tr("Error"), tr("Number of addresses involved in the address creation > 16\nPlease reduce the number"), QMessageBox::Ok);
        return;
    }

    if(required == 0)
    {
        ui->requiredSignatures->setFocus();  //QMessageBox::warning(this, tr("Error"), tr("Number of required signatures is 0\nNumber of required signatures must be between 1 and number of keys involved in the creation of address."), QMessageBox::Ok);
        return;
    }

    if(required > pubkeys.size())
    {
        ui->requiredSignatures->setFocus();   //QMessageBox::warning(this, tr("Error"), tr("Number of required signatures > Number of keys involved in the creation of address."), QMessageBox::Ok);
        return;
    }
    ...
    {
        q = tr("Redeem script exceeds size limit:");
		QString p = tr("Please reduce the number of public keys");
     //QMessageBox::warning(this, tr("Error"), tr("Redeem script exceeds size limit: %1 > %2\nReduce the number of public key involved in the address creation.").arg(script.size()).arg(MAX_SCRIPT_ELEMENT_SIZE), QMessageBox::Ok);
        QMessageBox::warning(this, tr("Error"), q + tr(" %1 > %2\n").arg(script.size()).arg(MAX_SCRIPT_ELEMENT_SIZE) + p, QMessageBox::Ok);
        return;
    }
    ...
    on_saveRedeemScriptButton_clicked();
    on_saveMultisigAddressButton_clicked();
}
...
bool MultisigDialog::saveRedeemScript(const std::string redeemScript)
{
    bool rzt = false;
    if( redeemScript.length() < 30 ){ return rzt; }
...
void MultisigDialog::on_saveRedeemScriptButton_clicked()
{
    ...
    saveRedeemScript(redeemScript);
/*    if( redeemScript.length() < 34 ){ return; }
...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    std::string label = strprintf("Multisig-%s", DateTimeStrFormat("%Y-%m-%d-%H-%M-%S", GetTime()).c_str());   //("multisig");
...
//    if( wallet->mapAddressBook.count( addr.Get() ) )  //if(!wallet->mapAddressBook.count(CBitcoinAddress(address)))
        wallet->SetAddressBookName(addr.Get(), label);   //wallet->SetAddressBookName(CBitcoinAddress(address), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                scriptPubKey.SetDestination(address.Get());   //scriptPubKey.SetAddress(address);
...
void MultisigDialog::on_signTransactionButton_clicked()
{
    ...
    for(unsigned int i = 0; i < mergedTx.vin.size(); i++)
    {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, mergedTx, i, 0))   //if(!VerifyScript(txin.scriptSig, prevPubKey, mergedTx, i, true, 0))
        {
            fComplete = false;
        }
    }
    ...
    {
        ui->statusLabel->setText(tr("Transaction is NOT completely signed,please copy the above signature text to another signer"));
        ui->sendTransactionButton->setEnabled(false);
    }
}
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    QString q = tr("The fee of the transaction"), qa = tr("Do you want to send the transaction anyway?");
    if(fee < minFee)
    {
        QString sm = tr("is smaller than the expected fee");
		QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), q + tr(" (%1 VPN) ").arg((double) fee / COIN) + sm + tr(" (%2 VPN). ").arg((double) minFee / COIN) + qa, QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    else if(fee > minFee)
    {
        QString s2 = tr("is bigger than the expected fee");
		QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), q + tr(" (%1 VPN) ").arg((double) fee / COIN) + s2 + tr(" (%2 VPN). ").arg((double) minFee / COIN) + qa, QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    ...
    //CTxDB txdb("r");
    if( !AcceptToMemoryPool(mempool, tx, NULL) )   //if(!tx.AcceptToMemoryPool(txdb, false))
    return;
...
void MultisigDialog::on_clearSpendBtn_clicked()
{
    while(ui->inputs->count())
        delete ui->inputs->takeAt(0)->widget();
    while(ui->outputs->count())
    ...
    addInput();
    addOutput();
    ui->transaction->setText("");   ui->signedTransaction->clear();
}
