...
QString TransactionDesc::GetTxMessage(const CWalletTx& wtx)
{
    ...
    if (!stxData.empty())
    {
        if( (stxData.find(BitChain_Head) != std::string::npos) || (stxData.find("@FROM=") != std::string::npos) || (stxData.find("@SUBJ=") != std::string::npos) || (stxData.find("@MSG=") != std::string::npos) || (stxData.find("BitNet") != std::string::npos) )
        {
			...
			if( sBcp.length() > 0 ){ return QString::fromStdString(sBcp); }
            else return stxData.c_str();
        }
        else
        {
...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    if (GetTxMessage(wtx) != NULL)
        strHTML += "<b>" + tr("Transaction Message") + ":</b><br> " + GetTxMessage(wtx) + "<hr><br>";
    ...
    if (fDebug)
    {
		...
		//CTransaction& tx = (CTransaction*)wtx; //(CTransaction*)&wtx;
		//unsigned int nBytes = wtx.GetSerializeSize(wtx, SER_NETWORK, PROTOCOL_VERSION);
		unsigned int nBytes = wtx.GetSerializeSize(SER_NETWORK, PROTOCOL_VERSION);
		std::string sSz = strprintf("Tx Size = %u", nBytes);
		strHTML += strHTML + "<li>"	+ QString::fromStdString(sSz) + "</li>";
