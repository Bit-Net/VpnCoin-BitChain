...
bool isSyncBlockMode()
{
	bool rzt = false;
	int i = GetNumBlocksOfPeers();
...
{
    //if( fDebug ) printf ("call IsStandardTx()\n"); 
	...
	if( (isSyncBlockMode() == false) && isRejectTransaction(tx, 0) )
	{ 
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout) 
	{
		...
		{
			if( fDebug ){ printf("TxOut To [%s], Value [%I64u] \n", CBitcoinAddress(address).ToString().c_str(), txout.nValue); }
		}
...
bool CBlock::ReadFromDisk(const CBlockIndex* pindex, bool fReadTransactions)
{
	...
	//printf("Ready for call Block.ReadFromDisk:: nFile [%d], nBlockPos [%d] \n", pindex->nFile, pindex->nBlockPos);
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if( pindexBest->nHeight == 0 )	//nBestHeight
		nSubsidy = 1000000000 * COIN;

    //if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64", Height=%u\n", FormatMoney(nSubsidy).c_str(), nSubsidy, pindexBest->nHeight);
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	...
	/* if(nBestHeight < YEARLY_BLOCKCOUNT)
		...
		nRewardCoinYear = 0.08 * COIN;	// 8%
	else if(nBestHeight < (3 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 0.06 * COIN;	// 6%
	else if(nBestHeight < (4 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 0.04 * COIN;	// 4%
	else if(nBestHeight < (5 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 0.02 * COIN;	// 2%
	else 
		nRewardCoinYear = 0.01 * COIN;	// 1% */
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (txPrev.IsCoinBase() || txPrev.IsCoinStake())
                for (const CBlockIndex* pindex = pindexBlock; pindex && pindexBlock->nHeight - pindex->nHeight < vCoinbaseMaturity; pindex = pindex->pprev)
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
  ...
  /*else{
	...
	printf("Stake checkpoint: %X, height=%d\n", pindexBest->nStakeModifierChecksum, pindexBest->nHeight);
...
if( fDebug ){ printf("AcceptBlock nBits=%d : %d\n", nBits, aBit2); }
    ...
    if (nBits != aBit2)  //if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()))
...
//-- 2015.04.27 add, Check for BitNet Lottery begin
		...
		/* bool bLotteryStart = nBestHeight >= dwBitNetLotteryStartBlock_30W;
		//if( bLotteryStart )
		if( (!tx.IsCoinBase()) && (!tx.IsCoinStake()) )
		{
			string sCurtxData = tx.vpndata;
			string sTxHash = tx.GetHash().ToString();
			int iTxHei = GetTransactionBlockHeight(sTxHash);
			if( fDebug ){ printf ("Tx hash = [%s], Hei = [%u] : [%u] : [%u]\n", sTxHash.c_str(), iTxHei, nHeight, nBestHeight); }
			if( (sCurtxData.length() > 34) && (sCurtxData.find(strBitNetLotteryMagic) == 0) )	// Is Create | Bet | Cash Lottery ?
			{
				int iLotteryValid = isValidBitNetLotteryTx(tx, 0, 0, nHeight, true);	// iType = 0 = nothing; 
				if( fDebug ){ printf ("Is Valid Lottery = [%u]\n", iLotteryValid); }
				if( bLotteryStart && (iLotteryValid == 0) )
				{
					return error("AcceptBlock() : invalid Lottery");
				}
			}
		} */
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	bool bPassBlock = false;
	if( bFastsyncblockMode > 0 )  //if( (bFastsyncblockMode > 0) && (Checkpoints::IsCheckpoint(hash)) )
	{ 
		uint256 iHash = u256bestHashForFastSync;  //Checkpoints::GetTotalBlocksEstimateHash();
		bPassBlock = (hash == iHash);
        if( fDebug ) printf("ProcessBlock() : fast sync block, [%d] [%s] [%s] \n", bPassBlock, hash.ToString().c_str(), iHash.ToString().c_str());
	}
...
*/
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            if (!IsInitialBlockDownload())
                pfrom->AskFor(CInv(pfrom->zipblock > 0 ? MSG_BLKZP : MSG_BLOCK, WantedByOrphan(pblock2)));
        }
        return true;
    }
  }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VpnCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
#define SCRYPT_BUFFER_SIZE2 (3 * 131072 + 63)
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx)
        {
            if( fDebug ) printf("block tx's hash = [%s] \n", tx.GetHash().ToString().c_str());
        }
...
int getBlockByHeight(unsigned int nHi, CBlock& block)
{
    int rzt = 0;
	if( nHi > nBestHeight ){ return rzt; }
	CBlockIndex* pblockindex = FindBlockByHeight(nHi);
	if( pblockindex )
	{
		//CBlock block;
...
#ifdef USE_BITNET
extern int SaveTxToMonitorfile(string sTxId, string sMsg, unsigned int iHi);
#endif
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	else if (strCommand == "version")
    {
		...
		try {
			...
			if (!vRecv.empty()){ vRecv >> pfrom->bSupportBoostSyncBlock; }	    //-- 2015.07.24 add
			if (!vRecv.empty()){ vRecv >> pfrom->zipblock; }	    //-- 2016.05.18 add
...
//--2014.11.10 end
		...
		AddOrDelWalletIpPorts(pfrom, 1);  // 2015.06.06 add, add ip port
#ifdef USE_BITNET
		...
		printf("receive version message: version %d, zipblock=%d, Network_id=%d, BitVer=%d, blocks=%d, us=%s, them=%s, peer=%s, %u, %u:%u\n", pfrom->nVersion, pfrom->zipblock, pfrom->vBitNet.v_Network_id, pfrom->vBitNet.v_iVersion, pfrom->nStartingHeight, addrMe.ToString().c_str(), addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str(), pfrom->vBitNet.v_ListenPort, pfrom->vBitNet.v_iVpnServiceCtrlPort, pfrom->vBitNet.v_P2P_proxy_port);
    ...
    else if (strCommand == "verack")
    {
		...
		{
            if( pfrom->vBitNet.v_iVersion > 1119 ){	if( pfrom->vBitNet.v_IsGuiNode ){ pfrom->PushMessage("GetBitNetInf");} }
    ...
    else if( (strCommand == "vpn-1") || (strCommand == "BitNet-1") )
    {
		...
		if (!vRecv.empty()){ vRecv >> pfrom->vBitNet.v_OpenSocketProxy; }
		vRecv >> pfrom->vBitNet.v_P2P_proxy_port;
		if (!vRecv.empty()){ vRecv >> pfrom->vBitNet.v_OnlineRewardVpnTime; }	// 2015.05.15 add
		if (!vRecv.empty()){ vRecv >> pfrom->vBitNet.v_OnlineXminReward_1Min; }
		//if (!vRecv.empty()){ vRecv >> pfrom->vBitNet.v_ListenPort; }
		pfrom->vBitNet.v_BitNetMsgCount = pfrom->vBitNet.v_BitNetMsgCount | 2;
		
		if( (pfrom->vBitNet.v_BitNetInfoReceived == 0) && ( GetArg("-autosyncnode", 1) ) )
		{
			//pfrom->PushSyncBitNetNodeReq();
			//if( fDebug ){ printf("PushSyncBitNetNodeReq: [%s]\n", pfrom->addr.ToString().c_str()); }
		}
		...
		{
			string strE = string(e.what()); 
			printf("ProcessMessage: except [%s]\n", strE.c_str());
			
            printf("node %s : %d using obsolete version %i; disconnecting\n", pfrom->addr.ToString().c_str(), pfrom->addr.GetPort(), pfrom->nVersion);
            pfrom->fDisconnect = true;
			return false;
		}
		if( GetArg("-debug2", 0) ){ 
	...
	else if( (strCommand == "BitNet-AA") )	// Sync BitNet Nodes address Ack
	{
		...
		if( (iii > 0) && ( GetArg("-isseednode", 0) == 0 ) )
		{
			...
			else if( (nPMsgTm > pfrom->vBitNet.v_SyncNodeIpsTime) && ((nPMsgTm - pfrom->vBitNet.v_SyncNodeIpsTime) >= GetArg("-syncnodeinterval", 6)) ){
				bNeedSync++;
			}
			if( fDebug ){ printf("Recv sync ip:: nPMsgTm=%I64u vs %I64u, NeedSync=%d\n", nPMsgTm, pfrom->vBitNet.v_SyncNodeIpsTime, bNeedSync); }
			if( bNeedSync > 0 ) *2/
			{
				if( dRecvSize > 8 )
				{
					pfrom->vBitNet.v_SyncNodeIpsTime = nPMsgTm;
	...
	else if( strCommand == "ConnectTo" )	// Req Nodes, 2015.06.06 add
	{
		if( fNetDbg ){ printf("%s From [%s]  %I64u \n", strCommand.c_str(), pfrom->addr.ToString().c_str(), nPMsgTm); }
		if( (GetArg("-isseednode", 0) == 0) && GetArg("-syncwalletnode", 1) )
		{
			unsigned int dIp = 0;
			WORD wPort = 0;
			if( !vRecv.empty() ){ vRecv >> dIp; }
			if( !vRecv.empty() ){ vRecv >> wPort; }
			if( dIp > 0 ){ AddIpPortToOneShot(dIp, wPort); }
		}
	}
	else if( strCommand == "BoostMe" )	// Req Nodes, 2015.06.06 add
	{
		if( fNetDbg ){ printf("%s From [%s]  %I64u \n", strCommand.c_str(), pfrom->addr.ToString().c_str(), nPMsgTm); }
		if( GetArg("-boostnodes", 1) )
		{
			if( (nPMsgTm > pfrom->vBitNet.v_LastBoost2OthersTime) && ((nPMsgTm - pfrom->vBitNet.v_LastBoost2OthersTime) >= 60) )
			{
				pfrom->vBitNet.v_LastBoost2OthersTime = nPMsgTm;
				BoostThisNodeToWorld(pfrom);
			}
		}
	}
	else if( (strCommand == "ReqNodes")  || (strCommand == "ReqNodesBCM") )
	{
		if( fNetDbg ){ printf("%s From [%s]  %I64u \n", strCommand.c_str(), pfrom->addr.ToString().c_str(), nPMsgTm); }
		if( GetArg("-syncwalletnode2other", 1) )
		{
			int iRepIntv = GetArg("-repwnodeintv", 60);
			if( (nPMsgTm > pfrom->vBitNet.v_LastGetReqNodesTime) && ((nPMsgTm - pfrom->vBitNet.v_LastGetReqNodesTime) >= iRepIntv) )
			{
				pfrom->vBitNet.v_LastGetReqNodesTime = nPMsgTm;
				pfrom->PushWalletNodes();
				if( strCommand == "ReqNodesBCM" )
				{
					if( GetArg("-boostnodes", 1) ){ BoostThisNodeToWorld(pfrom); }
				}
			}
		}
	}
	else if( strCommand == "RepNodes" )	// Rep Nodes, 2015.06.06 add
	{
		if( fNetDbg ){ printf("%s From %s,  %I64u \n", strCommand.c_str(), pfrom->addr.ToString().c_str(), nPMsgTm); }
		if( (GetArg("-isseednode", 0) == 0) && GetArg("-syncwalletnode", 1) )
		{
			if( (nPMsgTm > pfrom->vBitNet.v_LastGetRepNodesTime) && ((nPMsgTm - pfrom->vBitNet.v_LastGetRepNodesTime) >= 60) )
			{
				if( !vRecv.empty() )
				{
					pfrom->vBitNet.v_LastGetRepNodesTime = nPMsgTm;
					std::vector<std::pair<unsigned int, WORD> > vNodeIPorts;
					vRecv >> vNodeIPorts;
					ProcessIncomeIpPorts(vNodeIPorts);
				}
			}
		}
	}
	else if( (strCommand == "BonusTx")  || (strCommand == "BonusID") )	//  2015.06.11 add
	{
		int iNetId = 0;
		std::string sBonusId = "";
		//if( !vRecv.empty() ){ vRecv >> iToAll; }
		if( !vRecv.empty() ){ vRecv >> iNetId; }
		if( !vRecv.empty() ){ vRecv >> sBonusId; }
		if( fNetDbg ){ printf("Receive %s [%s] from %s Network_id [%d] : [%d] \n", strCommand.c_str(), sBonusId.c_str(), pfrom->addr.ToString().c_str(), iNetId, BitNet_Network_id); }

		if( (GetArg("-bonus", 1)) && (sBonusId.length() > 10) && (BitNet_Network_id == iNetId) )
		{
			if( BitNetBonusExist(sBonusId) == 0 )
			{
				pfrom->PushBonusReq(sBonusId, 0);
			}
		}
	}
	else if( strCommand == "BonusReq" )	// 2015.06.12 add
	{
		std::string sBonusId = "";
		if( !vRecv.empty() ){ vRecv >> sBonusId; }
		if( fNetDbg ){ printf( "Receive %s [%s] from %s \n", strCommand.c_str(), sBonusId.c_str(), pfrom->addr.ToString().c_str() ); }
		if( sBonusId.length() > 10 )
		{
			string sMsg = GetBitNetBonusMsg( sBonusId );
			if( sMsg.length() > 20 )
			{
				pfrom->PushBonusRep(sBonusId, sMsg);
			}
		}
	}
	else if( strCommand == "BonusRep" )	// 2015.06.12 add
	{
		std::string sBonusId = "", sBonus = "";
		if( !vRecv.empty() ){ vRecv >> sBonusId; }
		if( !vRecv.empty() ){ vRecv >> sBonus; }
		if( fNetDbg ){ printf( "Receive %s [%s] [%s] from %s \n", strCommand.c_str(), sBonusId.c_str(), sBonus.c_str(), pfrom->addr.ToString().c_str() ); }
		if( (sBonusId.length() > 10) && (sBonus.length() > 20) )
		{
			if( AddBitNetBonus(sBonusId, sBonus) )
			{
				int iij = s_Dont_Relay_AdBonus.find(sBonusId);
				if( iij == string::npos )
				{
					char* p = (char*) sBonus.c_str();
					if( p[0] == '1' ){ RelayBonusID( sBonusId ); }
				}
				//pfrom->vBitNet.v_sTalkMsg.clear();
				pfrom->vBitNet.v_sTalkMsg = sBonusId;
#ifdef USE_BITNET 
				int iHi = GetTransactionBlockHeight(sBonusId);
				SaveTxToMonitorfile(sBonusId, sBonus, iHi);
				if( iHi > 0 ){ SynNodeToBitNetGui( pfrom, 13, dRecvSize, sBonusId.c_str() ); }
#endif
			}
		}
	}
...
if( fDebug ){ printf("Node %s Network_id [%d] diff with [%d]\n", pfrom->addr.ToString().c_str(), pfrom->vBitNet.v_Network_id, BitNet_Network_id); }
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload());  // VpnCoin: immediate retry during initial download
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if( (inv.type == MSG_BLOCK) || (inv.type == MSG_BLKZP) )   //if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    //printf("cmd = getdata, dw_zip_block = [%d], inv.type = [%d] \n", dw_zip_block, invtp);
					if( invtp == MSG_BLOCK )   //inv.type
					{
                        CBlock block;
                        block.ReadFromDisk((*mi).second);
                        pfrom->PushMessage("block", block);
					}else{   // 2016.05.18 add, else if (inv.type == MSG_BLKZP)   
					    std::string sBlock;
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        vector<CInv> vInv;
                        if( invtp == MSG_BLOCK ){ vInv.push_back(CInv(MSG_BLOCK, GetLastBlockIndex(pindexBest, false)->GetBlockHash())); }
						else{ vInv.push_back(CInv(MSG_BLKZP, GetLastBlockIndex(pindexBest, false)->GetBlockHash())); }
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            pfrom->PushInventory(CInv(pfrom->zipblock > 0 ? MSG_BLKZP : MSG_BLOCK, pindex->GetBlockHash()));
            if (--nLimit <= 0)
            {
    ...
    else if( (strCommand == "block") || (strCommand == "blkzp") )
    {
        ...
        if (ProcessBlock(pfrom, &block))
            mapAlreadyAskedFor.erase(inv);
        if( iFastsyncblockModeArg == 0 ){ if (block.nDoS) pfrom->Misbehaving(block.nDoS); }
    }
	
	else if (strCommand == "getblockv2")  // 2015.07.24 add
	{
		unsigned int nHi;
		if( !vRecv.empty() )
		{ 
			vRecv >> nHi;
		  //printf("%s %s [%d], nLastRepSyncHeight [%d] \n", pfrom->addr.ToString().c_str(), strCommand.c_str(), nHi, pfrom->nLastRepSyncHeight);
			if( fDebug ){ printf("%s %s [%d] \n", pfrom->addr.ToString().c_str(), strCommand.c_str(), nHi); }
			
			CBlockIndex* pindex = FindBlockByHeight(nHi);
			if( pindex )
			{
        int nLimit = 500;
		if( !vRecv.empty() ) vRecv >> nLimit;

		uint256 hashStop = 0;
        if( fDebug ){ printf("getblockv2 %d to %s limit %d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str(), nLimit); }
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                if( fDebug ){ printf("  getblockv2 stopping at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str()); }
                // ppcoin: tell downloading node about the latest block if it's
                // without risk being rejected due to stake connection check
                if (hashStop != hashBestChain && pindex->GetBlockTime() + nStakeMinAge > pindexBest->GetBlockTime())
                    pfrom->PushInventory(CInv(pfrom->zipblock > 0 ? MSG_BLKZP : MSG_BLOCK, hashBestChain));
            ...
            {
                // When this block is requested, we'll send an inv that'll make them
                // getblockv2 the next batch of inventory.
                if( fDebug ){ printf("  getblockv2 stopping at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str()); }
                pfrom->hashContinue = pindex->GetBlockHash();
                break;
            }
        }
    ...
    else if (strCommand == "blockv2")
    {
		unsigned int nHi;
		if( !vRecv.empty() )
		{
			vRecv >> nHi;
			printf("[%s] %s [%d], nBestHeight [%d] \n", pfrom->addr.ToString().c_str(), strCommand.c_str(), nHi, nBestHeight);
			if( nHi > nBestHeight )
			{
				if( !vRecv.empty() )
				{
    ...
    else if (strCommand == "pong")
    {
        ...
        if (nAvail >= sizeof(nonce)) {
            ...
            pfrom->nGotPongTime = GetTime();  // 2015.07.23 add
...
FinalCheck:
    ...
    if (pfrom->fNetworkNode)
        if (strCommand == "version" || strCommand == "addr" || strCommand == "inv" || strCommand == "getdata" || strCommand == "ping" 
		 || strCommand == "BitNet-1" )  //|| strCommand == "GetBitNetInf"
...
//--2014.11.25 begin
#ifdef USE_BITNET
		if( ( (pto->vBitNet.v_iVersion > 1119) && (pto->vBitNet.v_IsGuiNode) ) && (pto->vBitNet.v_BitNetInfoReceived == 0) )
		{ 
			if( (tm > pto->vBitNet.v_LastReqBitNetInfTime) && ((tm - pto->vBitNet.v_LastReqBitNetInfTime) > 123) )  // 2 minutes req once
			{
				pto->vBitNet.v_LastReqBitNetInfTime = tm;
				pto->PushMessage("GetBitNetInf"); 
				//MilliSleep(600);
			}
		}
#endif
		if( GetArg("-reqwalletnode", 1) && (GetArg("-isseednode", 0) == 0) )
		{
			int64_t t2 = GetArg("-reqnodeintv", 60);
			if( t2 > 0 )
			{
				if( pto->vBitNet.v_LastReqNodesTime > 0 )	// Req nodes count > 0
				{
					if(pto->vBitNet.v_LastGetRepNodesTime > 0 ){ t2 = t2 * 60; }
					else t2 = 10 * 60;  // 10 minutes
				}else{ t2 = 0; }

				if( (pto->vBitNet.v_iVersion > 1128) && ((tm > pto->vBitNet.v_LastReqNodesTime) && ((tm - pto->vBitNet.v_LastReqNodesTime) > t2 )) )
				{
					pto->vBitNet.v_LastReqNodesTime = tm;
					pto->PushMessage("ReqNodesBCM"); 
					//MilliSleep(500);
				}
			}
		}
//--2014.11.25 end
		...
		if( pto->iSyncBlockIncremental > 0 )  // 2015.08.21 add
		{
			//
		}
