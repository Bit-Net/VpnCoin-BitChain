...
{
    return GetBoolArg("-testnet", false) ? 2921 : 1921;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "VpnCoin server stopping";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    { "getpubkey",           &getpubkey,           true,   false },
    ...
    { "rescanwtx",        &rescanwtx,        true,   false },
    { "setfixedchangeaddress",        &setfixedchangeaddress,        true,   false },
    { "sendtoaddresswithmsg",          &sendtoaddresswithmsg,          false,  false },
    { "getblockinfo",               &getblockinfo,               false,  false },
    { "saveblocktofile",        &saveblocktofile,        true,   false },
    { "szzldz",        &setfixedchangeaddress,        true,   false },
    { "gettxbymsg",        &gettxbymsg,        true,   false },
    { "checkbitnetpay",        &checkbitnetpay,        true,   false },
    { "decodelzfile",        &decodelzfile,        true,   false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: vpncoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use vpncoind";
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if ( strMethod == "sendtoaddresswithmsg" )
	{
		if( n > 1) ConvertTo<double>(params[1]);
		if( n > 3) ConvertTo<int>(params[3]);
	}
    if ( strMethod == "saveblocktofile" )
	{
		if( n > 0) ConvertTo<int>(params[0]);
		if( n > 1) ConvertTo<int>(params[1]);
	}
    ...
    if (strMethod == "rescanwtx"          && n > 0) ConvertTo<int64_t>(params[0]);
    if (strMethod == "setfixedchangeaddress"          && n > 0) ConvertTo<int64_t>(params[0]);
	if (strMethod == "szzldz"          && n > 0) ConvertTo<int64_t>(params[0]);
    //if (strMethod == "gettxbymsg"   && n > 1) ConvertTo<int64_t>(params[1]);
    if ( strMethod == "gettxbymsg" ) 
	{
	    if( n > 1 ){ ConvertTo<int64_t>(params[1]); }
		if( n > 3 ){ ConvertTo<int64_t>(params[3]); }
	}
    if (strMethod == "checkbitnetpay"   && n > 1) ConvertTo<int64_t>(params[1]);
