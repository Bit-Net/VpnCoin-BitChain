    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s VPN %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
//if( fDebug ){ printf("[%s] nDebit = [%I64u], nFee = [%I64u]\n", strSentAccount.c_str(), nDebit, nFee); }
...
//if( fDebug ){ printf("[%s] nDebit = [%I64u], txout.nValue = [%I64u], %u\n", strSentAccount.c_str(), nDebit, txout.nValue, fIsMine); }
...
// <-- 2016.05.17 +++
		...
		if ( ExtractDestination(pcoin->vout[i].scriptPubKey, address, &txotp) )
		{
			//if( fDebug ){ printf("SelectCoinsMinConf():: txOut To [%s], Value [%I64u], type = [%d] \n", CBitcoinAddress(address).ToString().c_str(), n, txotp); }
			if( TX_SCRIPTHASH == txotp ){ continue; }
		}
//-- 2016.05.17 --->
...
#endif
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& stxData, int bEncrypt, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
    ...
    /* if (coinControl && coinControl->HasSelected()){
		if( fDebug ){ printf("coinControl [%s]\n", coinControl); }	
	}
	ToString */
	
	CCoinControl* vCoinControl = (CCoinControl*) coinControl;
	
    // Transaction data
    if( !stxData.empty() )  //if (!stxData.empty() && stxData.length() <= MAX_TX_DATA)
    {
		if( bEncrypt > 0 )  //-2015.03.18 add
		{
#ifdef USE_CRYPT
        SimpleCrypt processSimpleCrypt((quint64)wtxNew.nTime);
        stxData = processSimpleCrypt.encryptToString(QString(stxData.c_str())).toStdString();
#endif
		}
        try {
            wtxNew.vpndata.resize(0);  //clear();
            //wtxNew.vpndata = stxData;
			int nDatLen = stxData.length();
            wtxNew.vpndata.resize(nDatLen);
			memcpy((char*)wtxNew.vpndata.c_str(), (char*)stxData.c_str(), nDatLen);
        }
...
#ifdef USE_BITNET
			std::string sAddr = "";
			if( (vCoinControl == NULL) && (wtxNew.strFromAccount.length() > 0) ){
				if( wtxNew.strFromAccount.length() == 34 ){
					sAddr = wtxNew.strFromAccount;
				}else{
					sAddr = getAddressesbyaccount( wtxNew.strFromAccount );
				}
				if( sAddr.length() == 34 ){	
					vCoinControl = new CCoinControl();   bCrtCC = true;
				
					if( fDebug ){ printf("CreateTransaction [%s] : [%I64u]\n", s_Cur_Cheque_TxID.c_str(), i6_Cur_Cheque_Amount); }					
					if( s_Cur_Cheque_TxID.length() > 34 )	//--2015.02.18 add
					{
						int idx = GetCoinAddrInTxOutIndex(s_Cur_Cheque_TxID, sAddr, i6_Cur_Cheque_Amount);
						COutPoint outpt(uint256(s_Cur_Cheque_TxID), idx);
						vCoinControl->Select(outpt);
						if( fDebug ){ outpt.print(); }
					}
					vCoinControl->destChange = CBitcoinAddress(sAddr).Get();					
				}
			}
#endif
//if( fDebug ){ printf("CreateTransaction 1 [%u] [%s] [%s] > [%u], [%I64u]\n", wtxNew.strFromAccount.length(), wtxNew.strFromAccount.c_str(), sAddr.c_str(), vCoinControl, nValue); }
            ...
            while (true)
            {
                ...
                int64_t nValueIn = 0;
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, vCoinControl))
				{
                    if( bCrtCC ){ delete vCoinControl; }
					return false;
				}
...
//if( fDebug ){ printf("CreateTransaction 2 nChange: [%I64u] : [%I64u] : [%I64u] : [%I64u]\n", nChange, nValueIn, nValue, nFeeRet); }
                if (nChange > 0)
                {
					...
					/* if( IsSpecifiedDest > 0 ){	//-- 2015.02.06 add
						scriptChange.SetDestination(vDestChange);
					}
					
                    // coin control: send change to custom address
                    else */ 
					if (vCoinControl && !boost::get<CNoDestination>(&vCoinControl->destChange))
                        scriptChange.SetDestination(vCoinControl->destChange);					
                    ...
                    else
                    {
                        ...
                        CPubKey vchPubKey;
						//--2015.07.01 add
						if( fFixedChangeAddress > 0 )
						{
							if( this->vchDefaultKey.IsValid() ){ vchPubKey = this->vchDefaultKey; }  //pwalletMain
							else{ assert(reservekey.GetReservedKey(vchPubKey)); }
						}
						else{
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& stxData, int bEncrypt, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, stxData, bEncrypt, coinControl);
}
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckStakeKernelHash(pindexPrev, nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, targetProofOfStake))
            {
                ...
                if( GetArg("-staketodefault", 0) )
                {
                    //CPubKey pubkey;
					scriptPubKeyKernel = CScript() << vchDefaultKey << OP_CHECKSIG;
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, std::string& stxData, int bEncrypt, bool fAskFee)
{
    vector<pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return SendMoney(vecSend, nValue, wtxNew, stxData, bEncrypt, fAskFee);
 /*   CReserveKey reservekey(this);
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, stxData, bEncrypt))	//std::string()))
    {
        string strError;
...
string CWallet::SendMoney(const vector<pair<CScript, int64_t> >& vecSend, int64_t nValue, CWalletTx& wtxNew, std::string& stxData, int bEncrypt, bool fAskFee)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
    ...
    if (!CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, stxData, bEncrypt))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds  "), FormatMoney(nFeeRequired).c_str());
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, CWalletTx& wtxNew, std::string& stxData, int bEncrypt, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, wtxNew, stxData, bEncrypt, fAskFee);
}
